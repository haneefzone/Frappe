...
# - Adds SQL + Redis + Frappe Bench autostart lines into /root/.bashrc
#
...
detect_tz() {
  # 1) If TZ already set (e.g. via docker -e TZ=...), use it
  if [ -n "$TZ" ]; then
    echo "$TZ"
    return
  fi

  # 2) If /etc/timezone exists, use it
  if [ -f /etc/timezone ]; then
    cat /etc/timezone
    return
  fi

  # 3) If /etc/localtime symlink points into zoneinfo, derive TZ
  if [ -L /etc/localtime ]; then
    realpath /etc/localtime | sed 's|^/usr/share/zoneinfo/||'
    return
  fi
...
echo "Configuring Supervisor for Frappe bench auto-start ..."
...
Frappe bench will auto-start on container restart via Supervisor.

Manual control:
  supervisorctl status frappe-bench
...
###########################################################
#  Auto-start MariaDB + Redis + Supervisor on root shell
###########################################################

BASHRC="/root/.bashrc"

STARTUP_CMDS=$(cat <<'EOF'
# Auto-start MariaDB, Redis, and Supervisor (for Frappe) on container start
...
echo "Configuring automatic services autostart in /root/.bashrc ..."

if grep -q "service mariadb start" "$BASHRC"; then
    echo "Autostart commands already exist in /root/.bashrc"
else
    echo "" >> "$BASHRC"
    echo "$STARTUP_CMDS" >> "$BASHRC"
    echo "Autostart commands added to /root/.bashrc"
fi
